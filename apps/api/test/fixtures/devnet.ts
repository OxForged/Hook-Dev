 ...
 * ============================================================================
 *
 * Every value in this file is invented, and it is TEST DATA ONLY: it lives under
 * test/, nothing in src/ imports it, and the API serves only indexed chain data.
 * (Historical note: this used to be the API's default data source, from before
 * Latch was deployed. Latch has been live on Robinhood Chain since 2026-09-11.)
 *
 * Guard rails, so a fixture value can never be mistaken for an observation:
 *
 *   1. The chain is id 31337, named "Fixture Devnet (not a real network)".
 *   2. Every address begins with the marker nibbles `0xf1c7` ("fict").
 *   3. Token symbols are prefixed `FIX`.
 *
 * Real chain data for tests lives beside this file in
 * robinhood-swap-0x68286e9b.json (a receipt read from Robinhood Chain).
 *
...
import type { RawLog } from "../../src/chain/decode.js";

/*
 * TEST DATA ONLY. Moved out of src/ when the API went live on Robinhood: no
 * production module imports this file, and the build (tsconfig.build.json)
 * compiles src/ only. It exercises the decoder against synthetic but really
 * ABI-encoded logs.
 */

export type ContractRole = "Vault" | "CLPoolManager" | "BinPoolManager" | "FeeController";

/** V1 fee-controller events, used only to build fixture logs. */
const FEE_CONTROLLER_EVENTS_ABI = parseAbi([
...
export interface FixtureLog extends RawLog {
...
function encode(event: FixtureEvent): FixtureLog {
  ...
  return {
    ...
    data,
    blockNumber,
    blockHash: keccak256(new TextEncoder().encode(`latchprotocol-fixture-block-${event.block}`)),
...
function buildEvents(): FixtureEvent[] {
  ...
  for (let i = 0; i < 240; i++) {
    ...
    // Signed from the CALLER's point of view, as the real `Swap` event emits it:
    // the input is paid in (-), the output is received (+). Verified on 4663, tx
    // 0x68286e9b…629a (amount0 = -1e18 while 1e18 of currency0 entered the Vault).
...
let cachedLogs: FixtureLog[] | undefined;

/** Every fixture log, ordered by (blockNumber, logIndex). Built once. */
export function fixtureLogs(): readonly FixtureLog[] {
  if (!cachedLogs) {
