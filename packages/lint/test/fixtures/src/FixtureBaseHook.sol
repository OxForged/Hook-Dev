...
/// @notice The base-hook shape every real hook framework uses: the pool manager
/// enters through a `final` external callback which forwards to an internal
/// override point, and the base's default reverts so an un-overridden
/// permission fails loudly.
///
/// Fixtures inherit from this so the tests exercise the linter's delegation
/// resolution, not a flattened single-contract shortcut.
abstract contract FixtureBaseHook {
    ...
    function _noSwapChange() internal pure returns (bytes4, BeforeSwapDelta, uint24) {
        return (IHooks.beforeSwap.selector, BeforeSwapDeltaLibrary.ZERO_DELTA, 0);
    }

    function _noLiquidityChange(bytes4 selector) internal pure returns (bytes4, BalanceDelta) {
